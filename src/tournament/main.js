...
Object.assign(window, {
  tShowTab, tOnEventChange, tOpenAuth, tCloseAuth, tHandleLogin, tHandleLogout,
  tTambahGrup, tHapusGrup, tTambahPemain, tToggleStatusPemain,
  tOpenGroupMatchForm, tSubmitGroupMatch, tOpenKnockoutMatchForm, tSubmitKnockoutMatch, tSuggestUrutan,
  tFilterJadwal, tInputSkor, tCloseSkorModal, tTambahSetRow, tHapusSetRow, tSubmitSkor, tHapusMatch,
  tOpenEditPemain, tCloseEditPemain, tSubmitEditPemain,
  tRunH2H, tBukaEventBaru, tSubmitEventBaru, tUbahStatusEvent,
});
...
function _toggleAdminUI(isAdmin) {
  document.querySelectorAll('.t-admin-only').forEach((el) => el.classList.toggle('hidden', !isAdmin));
...
async function loadEvents() {
  ...
  const activeEvent = events.find((e) => e.status === 'AKTIF') || events[0] || null;
...
async function tSubmitEventBaru() {
  ...
  const tahun = Number(document.getElementById('t-event-tahun').value) || new Date().getFullYear();
  const msg = document.getElementById('t-event-msg');
  if (!nama) { msg.textContent = 'Nama turnamen wajib diisi.'; return; }
  try {
...
function tShowTab(tab) {
  ...
  document.querySelectorAll('.t-tab-btn').forEach((b) => b.classList.remove('t-tab-active'));
  document.querySelectorAll('.t-content-section').forEach((s) => s.classList.add('hidden'));
...
function _renderAll() {
  const { grupList, pemainList, matchList, activeTab, jadwalFilter, isAdmin } = store.getState();
  if (activeTab === 'grup') {
    document.getElementById('t-tab-grup').innerHTML = renderFaseGrup(grupList, pemainList, matchList);
  } else if (activeTab === 'jadwal') {
    document.getElementById('t-jadwal-list').innerHTML = renderJadwalHasil(matchList, pemainList, grupList, jadwalFilter, isAdmin);
  } else if (activeTab === 'bracket') {
    document.getElementById('t-bracket-list').innerHTML = renderBracket(matchList, pemainList, isAdmin);
...
function tFilterJadwal(filter) {
  ...
  document.querySelectorAll('.t-filter-btn').forEach((b) => b.classList.remove('t-filter-active'));
  document.getElementById('t-filter-' + filter)?.classList.add('t-filter-active');
  _renderAll();
}
...
// AUTH
// ================================================================
function tOpenAuth() { document.getElementById('t-auth-overlay').classList.remove('hidden'); }
function tCloseAuth() { document.getElementById('t-auth-overlay').classList.add('hidden'); }
...
function _renderKelola() {
  ...
  document.getElementById('t-grup-admin-list').innerHTML = renderGrupAdminList(grupList);
  document.getElementById('t-pemain-admin-list').innerHTML = renderPemainAdminList(pemainList, grupList);
  _fillAdminSelects();
}
...
function _fillAdminSelects() {
  ...
  ['t-pemain-grup', 't-gm-grup'].forEach((id) => {
    ...
    if (el) el.innerHTML = grupOpts || '<option disabled>Belum ada grup</option>';
  });
  ['t-gm-p1', 't-gm-p2'].forEach((id) => {
    const el = document.getElementById(id);
    if (el) el.innerHTML = pemainOpts || '<option disabled>Belum ada pemain aktif</option>';
  });
  // Match babak gugur & edit pemain boleh kosong (TBD)
  ['t-ko-p1', 't-ko-p2', 't-edit-p1', 't-edit-p2'].forEach((id) => {
    const el = document.getElementById(id);
    if (el) el.innerHTML = pemainOpts ? pemainOptsTBD : '<option value="">— TBD —</option>';
  });
}
...
async function tTambahPemain() {
  ...
  await logService.record(user?.email, 'tambah_pemain', { nama }, eventAktif.id);
...
async function tToggleStatusPemain(id, statusSekarang) {
  ...
  await logService.record(user?.email, statusBaru === 'AKTIF' ? 'aktif_pemain' : 'nonaktif_pemain', { id }, eventAktif.id);
  await loadEventData(eventAktif.id);
}
...
// ================================================================
function tOpenGroupMatchForm() { document.getElementById('t-modal-gm').classList.remove('hidden'); }
async function tSubmitGroupMatch() {
  ...
  if (!grup_id || !pemain1_id || !pemain2_id || pemain1_id === pemain2_id) {
    msg.textContent = 'Pilih grup dan dua pemain berbeda.'; return;
  }
  await matchService.createGroupMatch(eventAktif.id, { grup_id, pemain1_id, pemain2_id, tanggal, tempat });
  await logService.record(user?.email, 'tambah_jadwal_grup', { pemain1_id, pemain2_id }, eventAktif.id);
...
async function tSuggestUrutan() {
  ...
  } catch (e) { /* biarkan nilai lama kalau gagal fetch */ }
}
...
async function tSubmitKnockoutMatch() {
  ...
  const urutan_bracket = Number(document.getElementById('t-ko-urutan').value) || 0;
  ...
  if (!pemain1_id && !pemain2_id) {
    msg.textContent = 'Isi minimal salah satu pemain (boleh salah satunya TBD).'; return;
  }
  if (pemain1_id && pemain1_id === pemain2_id) {
    msg.textContent = 'Pemain 1 dan Pemain 2 tidak boleh sama.'; return;
  }
  await matchService.createKnockoutMatch(eventAktif.id, { ronde, urutan_bracket, pemain1_id, pemain2_id, tanggal, tempat });
  await logService.record(user?.email, 'tambah_jadwal_gugur', { ronde, pemain1_id, pemain2_id }, eventAktif.id);
...
function tCloseEditPemain() { document.getElementById('t-modal-edit-pemain').classList.add('hidden'); }
...
async function tSubmitEditPemain() {
  ...
  if (!pemain1_id && !pemain2_id) { msg.textContent = 'Minimal salah satu pemain harus diisi.'; return; }
  if (pemain1_id && pemain1_id === pemain2_id) { msg.textContent = 'Pemain 1 dan Pemain 2 tidak boleh sama.'; return; }
  const { eventAktif, user } = store.getState();
  try {
    await matchService.updatePemain(editingPemainMatchId, { pemain1_id, pemain2_id });
    await logService.record(user?.email, 'edit_pemain_match', { matchId: editingPemainMatchId, pemain1_id, pemain2_id }, eventAktif.id);
    tCloseEditPemain();
    await loadEventData(eventAktif.id);
  } catch (e) {
...
function tInputSkor(matchId) {
  ...
  if (!m.pemain1_id || !m.pemain2_id) {
    alert('Lengkapi kedua pemain dulu (masih ada slot TBD) sebelum input skor. Pakai tombol ✏️ / "Edit Pemain" untuk mengisinya.');
    return;
  }
  skorSetRows = m.set_skor?.length ? JSON.parse(JSON.stringify(m.set_skor)) : [{ p1: '', p2: '' }];
  document.getElementById('t-modal-skor').classList.remove('hidden');
  _renderSetRows();
}
function tCloseSkorModal() { document.getElementById('t-modal-skor').classList.add('hidden'); }
function tTambahSetRow() { skorSetRows.push({ p1: '', p2: '' }); _renderSetRows(); }
function tHapusSetRow(idx) { skorSetRows.splice(idx, 1); _renderSetRows(); }

function _renderSetRows() {
  const el = document.getElementById('t-set-rows');
  el.innerHTML = skorSetRows.map((s, i) => `
...
async function tSubmitSkor() {
  ...
  if (!sets.length) { msg.textContent = 'Isi minimal 1 set.'; msg.classList.remove('hidden'); return; }
  const { eventAktif, user } = store.getState();
  try {
    await matchService.saveScore(editingMatchId, sets, user?.email);
    await logService.record(user?.email, 'input_skor', { matchId: editingMatchId, sets }, eventAktif.id);
    tCloseSkorModal();
    await loadEventData(eventAktif.id);
  } catch (e) {
    msg.textContent = e.message;
...
function tRunH2H() {
  ...
  if (!aId || !bId || aId === bId) {
    document.getElementById('t-h2h-result').innerHTML = '<div class="text-center text-red-500 text-sm py-4">Pilih dua pemain berbeda.</div>';
    return;
  }
  ...
  document.getElementById('t-h2h-result').innerHTML = renderH2HResult(namaA, namaB, hasil);
}
