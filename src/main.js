...
(async () => {
  try {
    ...
    sel.innerHTML = sesiList
      .map((s) => {
        const badge =
          s.status === "AKTIF" ? " 🟢" : s.status === "DRAFT" ? " 🟡" : " ⚪";
...
function renderAll(matches) {
  ...
  document.getElementById("list-jadwal").innerHTML = renderJadwal(
    matches,
    isAdmin,
  );
  document.getElementById("list-hasil").innerHTML = renderHasil(
    matches,
    isAdmin,
  );
  renderPodium(matches);
  document
    .querySelectorAll(".row-pemain")
    .forEach((r) =>
...
async function handleLogin() {
  ...
  if (!email) {
    showAuthMsg("Masukkan email.", "error");
    return;
  }
  if (!password) {
    showAuthMsg("Masukkan password.", "error");
    return;
  }
...
async function handleForgotPassword() {
  const email = document.getElementById("auth-email").value.trim();
  if (!email) {
    showAuthMsg("Isi email dulu.", "error");
    return;
  }
  try {
  ...
  const btn = (label, f, active) =>
    `<button onclick="openDetail('${nama}','${f}')"
      class="px-3 py-1 text-xs font-semibold rounded-lg transition-all ${
        active ? "bg-gray-100 text-slate-700" : "bg-gray-800 text-gray-400"
      }">${label}</button>`;
...
async function openModal(matchId = null) {
  ...
  document.getElementById("modal-skor-title").textContent = isEdit
    ? "Edit Skor"
    : "Input Skor";
  ...
  if (isEdit) {
    ...
    if (m) {
      ...
      if (m.tanggal)
        document.getElementById("inp-tgl").value = new Date(m.tanggal)
          .toISOString()
          .slice(0, 16);
      ["s1", "s2", "s3", "s4", "s5"].forEach((k) => {
        document.getElementById("inp-" + k).value = m[k] || "";
      });
      if (m.skor_akhir?.includes("W"))
        document.getElementById("inp-wo").value = m.skor_akhir;
    }
  } else {
    ...
    pendingMatches = await matchService.getPendingMatches(
...
function renderMatchList(matches) {
  ...
  if (!matches.length) {
    c.innerHTML =
      '<p class="p-4 text-center text-sm text-gray-400 italic">Tidak ada jadwal pending.</p>';
    return;
  }
  c.innerHTML = matches
    .map(
...
function filterMatchList() {
  ...
  renderMatchList(
    pendingMatches.filter(
      (m) =>
        m.pemain_1.toLowerCase().includes(kw) ||
        m.pemain_2.toLowerCase().includes(kw),
    ),
  );
}
...
async function submitSkor() {
  ...
  if (!matchId) {
    showSkorMsg("Pilih pertandingan terlebih dahulu!", "error");
    return;
  }

  const wo = document.getElementById("inp-wo").value;
  const sets = ["inp-s1", "inp-s2", "inp-s3", "inp-s4", "inp-s5"].map(
  ...
  try {
    ...
    await logService.log(
      ...
      {
        matchId,
        p1: matchBefore?.pemain_1,
        p2: matchBefore?.pemain_2,
...
async function hapusMatch(matchId) {
  ...
  if (
    !confirm(
      `Hapus pertandingan ${m?.pemain_1} vs ${m?.pemain_2}?\nTindakan ini tidak bisa dibatalkan.`,
    )
  )
    return;
  try {
...
function closeModalUbahJadwal() {
  document.getElementById("modalUbahJadwal").classList.add("hidden");
}
...
async function loadPemain() {
  ...
  list.innerHTML =
    '<div class="text-center py-6 text-gray-400 text-sm animate-pulse">Memuat...</div>';
  try {
    const pemain = await pemainService.getBySesi(sesiId);
    if (!pemain.length) {
      list.innerHTML =
        '<div class="text-center py-6 text-gray-400 text-sm">Belum ada pemain di sesi ini.</div>';
      return;
    }
    list.innerHTML = pemain
      .map(
        (p) => `
...
function closeModalPemain() {
  document.getElementById("modalPemain").classList.add("hidden");
}
...
async function submitPemain() {
  ...
  if (!nama) {
    showMsg("Nama tidak boleh kosong.", "error");
    return;
  }
  btn.disabled = true;
  btn.textContent = "Menyimpan...";
...
async function loadLog() {
  ...
  try {
    ...
    if (!logs.length) {
      list.innerHTML =
        '<div class="text-center py-6 text-gray-400 text-sm">Belum ada log aktivitas.</div>';
      return;
    }
    const BULAN = [
      "Jan",
...
async function jalankanPreview() {
  ...
  try {
    ...
    const jadwal = distribusiTanggal(
      pasangan,
      new Date(sesi.tgl_mulai),
      new Date(sesi.tgl_selesai),
      libur,
      genMatchPerHari,
    );
    ...
    genPreviewData = {
      ...
      jadwal,
      pemain,
      sudahAdaJadwal: pending.length > 0,
    };
...
async function konfirmasiSimpan() {
  ...
  if (
    genPreviewData.sudahAdaJadwal &&
    !document.getElementById("chk-overwrite").checked
  ) {
  ...
  try {
    ...
    await logService.log(
      "generate_jadwal",
      { totalMatch: jadwal.length, mode: genPreviewData.mode },
      sesiId,
    );
...
async function submitRegisterAdmin() {
  ...
  const showMsg = (t, type) => {
    ...
    el.textContent = t;
    el.className =
      "mb-3 p-2 rounded-lg text-xs text-center " +
      (type === "success"
        ? "bg-green-50 text-green-700"
        : "bg-red-50 text-red-700");
    el.classList.remove("hidden");
  ...
  if (!email) {
    showMsg("Masukkan email.", "error");
    return;
  }
  if (pw.length < 8) {
    showMsg("Password min. 8 karakter.", "error");
    return;
  }
  if (pw !== confirm) {
    showMsg("Password tidak cocok.", "error");
    return;
  }
...
function updateInfoPoin(sesiId) {
  ...
  document.getElementById("label-poin-menang").textContent = isLanjut
    ? "+3 Poin"
    : "+2 Poin";
  document.getElementById("label-poin-kalah").textContent = isLanjut
    ? "0 Poin"
    : "+1 Poin";
  document.getElementById("label-poin-wo").textContent = isLanjut
    ? "-1 Poin"
    : "0 Poin";
}

function showSkeletonKlasemen() {
  const rows = Array.from(
    { length: 10 },
    () => `
    <tr class="animate-pulse">
      <td class="px-4 py-3"><div class="h-4 w-4 bg-gray-200 rounded mx-auto"></div></td>
      <td class="px-2 py-3"><div class="h-4 w-24 bg-gray-200 rounded"></div></td>
      <td class="px-2 py-3"><div class="h-4 w-6 bg-gray-200 rounded mx-auto"></div></td>
      <td class="px-2 py-3"><div class="h-4 w-6 bg-gray-200 rounded mx-auto"></div></td>
      <td class="px-2 py-3"><div class="h-4 w-6 bg-gray-200 rounded mx-auto"></div></td>
      <td class="px-2 py-3"><div class="h-4 w-6 bg-gray-200 rounded mx-auto"></div></td>
      <td class="px-3 py-3"><div class="h-4 w-8 bg-gray-200 rounded mx-auto"></div></td>
      <td class="px-2 py-3"><div class="h-4 w-10 bg-gray-200 rounded mx-auto"></div></td>
      <td class="px-2 py-3"><div class="h-4 w-10 bg-gray-200 rounded mx-auto"></div></td>
    </tr>
  `,
  ).join("");
  document.getElementById("body-klasemen").innerHTML = rows;
}
