...
import { hitungKlasemen, hitungDetailPlayer } from '../lib/klasemenCalculator.js';
...
export function renderJadwal(matches, isAdmin = false) {
  ...
  let html = '<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-3 p-2">';
...
export function renderHasil(matches, isAdmin = false) {
  ...
  for (const [tgl, items] of Object.entries(groups)) {
    ...
    items.forEach(m => {
      const { display: skorDisplay, badge, p1Bold, p2Bold } = _parseSkorHasil(m);
      html += `
        <div class="bg-white border rounded-xl p-4 shadow-sm mb-2">
          ...
          <div class="flex justify-center space-x-2 mt-1">
            ${[m.s1, m.s2, m.s3, m.s4, m.s5].filter(s => s && s.includes('-')).map(s =>
...
export function renderPodium(matches) {
  ...
  byId('stat-wo-nama').textContent        = mostWO.length   ? mostWO.map(p=>p.nama).join(', ')   : '-';
  byId('stat-wo-angka').textContent       = mostWO.length   ? mostWO[0].wo   + ' WO'   : '0 WO';
  byId('stat-loss-nama').textContent      = mostLoss.length ? mostLoss.map(p=>p.nama).join(', ') : '-';
  byId('stat-loss-angka').textContent     = mostLoss.length ? mostLoss[0].kalah + ' Kalah' : '0 Kalah';
}
...
export function renderDetailPemain(nama, allMatches, klasemenData) {
  ...
  const hasilHtml = detail.hasil.length
    ? detail.hasil.map(m => {
        const woUpper = String(m.skor || '').trim().toUpperCase();
        let skorTampil = m.skorTampil;
        let woBadge = '';
        if (woUpper === 'W-W') { skorTampil = 'DOUBLE WO'; }
        else if (WO_SET.has(woUpper)) { skorTampil += ' (WO)'; woBadge = ' WO'; }

        return `
          <div class="flex items-center justify-between p-3 bg-white border rounded-xl shadow-sm mb-2">
            <div>
              <div class="text-[10px] text-gray-400 uppercase font-bold tracking-tight">${m.tgl}</div>
  ...
  const wrHtml = activeSetStats.length
    ? activeSetStats.map(s => {
        const pct      = s.winrate;
        const barColor = pct >= 70 ? 'bg-green-500' : pct >= 50 ? 'bg-yellow-400' : 'bg-red-400';
...
function _parseSkorHasil(m) {
  const s = String(m.skor_akhir || '').trim().toUpperCase();
  if (s === 'W-0')  return { display: '3 - 0', badge: ' <span class="ml-1 text-[10px] px-1 py-1 bg-red-100 text-red-600 rounded font-bold">WO</span>', p1Bold: true,  p2Bold: false };
  if (s === '0-W')  return { display: '0 - 3', badge: ' <span class="ml-1 text-[10px] px-1 py-1 bg-red-100 text-red-600 rounded font-bold">WO</span>', p1Bold: false, p2Bold: true  };
  if (s === 'W-W')  return { display: 'DOUBLE WO', badge: '', p1Bold: false, p2Bold: false };
