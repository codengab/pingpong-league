...
export function hitungKlasemen(matches) {
  ...
  for (const row of matches) {
    const { pemain_1: p1, pemain_2: p2, skor_akhir, tanggal, s1, s2, s3, s4, s5 } = row;

    _ensurePlayer(klasemen, p1);
    _ensurePlayer(klasemen, p2);

    const skorRaw = String(skor_akhir || '').trim().toUpperCase();
    if (!skorRaw || skorRaw === '0-0') continue; // belum selesai

    const isWO = WO_VALUES.has(skorRaw);
    let p1Poin = 0, p2Poin = 0;
    let p1Set  = 0, p2Set  = 0;
    let p1Bola = 0, p2Bola = 0;
...
export function hitungDetailPlayer(nama, matches) {
  ...
  const jadwal = [];
  const h2hMap = {};
  const setStats = Array.from({ length: 5 }, () => ({ main: 0, menang: 0, poinFor: 0, poinAgainst: 0 }));

  for (const row of matches) {
    const { pemain_1, pemain_2, skor_akhir, tanggal, s1, s2, s3, s4, s5, sesi_id } = row;
    ...
    if (isSelesai) {
      ...
      const tglDisplay = sesi_id ? `${tglIndo} <span class="text-[9px] bg-gray-100 text-gray-400 px-1 rounded">Sesi ${sesi_id}</span>` : tglIndo;

      hasil.push({
        lawan, skor: skor_akhir, skorTampil,
        tgl: tglDisplay,
        tglPlain: tglIndo,
  ...
  return {
    ...
    h2h:    Object.values(h2hMap).sort((a, b) => (b.menang + b.kalah) - (a.menang + a.kalah)),
...
function _ensurePlayer(map, nama) {
  if (nama && !map[nama]) {
    map[nama] = {
      nama, main: 0, menang: 0, kalah: 0, wo: 0,
      poin: 0, setM: 0, setK: 0, bolaM: 0, bolaK: 0, rawForm: [],
    };
  }
}
